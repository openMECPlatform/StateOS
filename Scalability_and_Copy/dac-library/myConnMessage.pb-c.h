...
typedef struct _UnlockMsg UnlockMsg;
...
typedef enum _MyConnMessage__DataType {
  ...
  MY_CONN_MESSAGE__DATA_TYPE__UnlockMsgType = 7
    PROTOBUF_C__FORCE_ENUM_TO_BE_INT_SIZE(MY_CONN_MESSAGE__DATA_TYPE)
} MyConnMessage__DataType;
typedef enum _ConnPutPerflowAckMsg__FwState {
  CONN_PUT_PERFLOW_ACK_MSG__FW_STATE__OPEN = 1,
  CONN_PUT_PERFLOW_ACK_MSG__FW_STATE__CLOSED = 2
    PROTOBUF_C__FORCE_ENUM_TO_BE_INT_SIZE(CONN_PUT_PERFLOW_ACK_MSG__FW_STATE)
...
typedef enum {
  ...
  MY_CONN_MESSAGE__MESSAGE_UNLOCK_MSG = 8
...
#define UNLOCK_MSG__INIT \
 { PROTOBUF_C_MESSAGE_INIT (&unlock_msg__descriptor) \
     }
...
typedef void (*ConnPutPerflowAckMsg_Closure)
                 (const ConnPutPerflowAckMsg *message,
                  void *closure_data);
typedef void (*UnlockMsg_Closure)
                 (const UnlockMsg *message,
                  void *closure_data);
...
extern const ProtobufCEnumDescriptor    conn_put_perflow_ack_msg__fw_state__descriptor;
extern const ProtobufCMessageDescriptor unlock_msg__descriptor;
