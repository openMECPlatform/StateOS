...
uint32_t *hash_nat_array[BUCKET_SIZE];
//+++
...
uint8_t external_mac[ETHER_ADDR_LEN];
...
void packet_print(const struct pcap_pkthdr *packet_header, const u_char *packet)
{
   ...
   printf("packet print\n");
    ...
    printf("%-4s packet print src=%u dst=%u\n", ip_protocol_ntoa(ip->protocol),
...
//+++
    ...
    uint32_t cxid = (uint16_t)((ip->ttl | 0x00 ) <<8) | (uint16_t)(ip->tos | 0x00 );
    //printf("cxid %u\n",cxid);
	...
	action_record = action_table_outbound(pi);
	//record = table_outbound(ip->saddr, eth->h_source, htons(*src_port),
        ...
        /* change source port */
        *src_port = ntohs(action_record->external_port);
	...
	//printf("begin to search action table\n");
        ...
        if ((conn_record = action_table_inbound(pi))) {

            /* change source MAC */
            memcpy(eth->h_source, interfaces[INTERFACE_INTERNAL].mac, ETH_ALEN);

            /* change dest MAC */
            memcpy(eth->h_dest, conn_record->internal_mac, ETH_ALEN);

            /* change dest IP */
            ip->daddr = conn_record->internal_ip;

            /* change dest port */
            *dst_port = ntohs(conn_record->internal_port);

            natted = 1;

            //printf("%-4s src=%u dst=%u\n", ip_protocol_ntoa(ip->protocol),
            //htons(tcp->source), htons(*dst_port));
        }
...
void nat_table_add(uint32_t hash , uint32_t nat_hash)
{
...
static void listen_interface(int interface_id)
{
    ...
    if(interface_id == INTERFACE_EXTERNAL){
    	memcpy(external_mac, interfaces[INTERFACE_EXTERNAL].mac, ETH_ALEN);
    }
...
int main(int argc, char **argv)
{
    ...
    memcpy(gateway_mac, gateway.mac, ETH_ALEN);
    
    printf("\n");
    CONNACLocals locals;
    bzero(&locals,sizeof(locals));
    locals.conn_get_perflow = &local_conn_get_perflow;
    locals.action_get_direct_perflow = &local_action_get_direct_perflow;
