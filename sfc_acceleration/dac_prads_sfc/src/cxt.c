...
static pthread_t action_send_thread;
...
actionState *send_action_bucket[BUCKET_SIZE];
int send_action;
...
int local_conn_get_one_perflow(connState* conn_state){	
	...
	conn_perflow->cxid = cxid;
        ...
        //printf("conn_perflow->hash %u",conn_perflow->hash);
        //printf("conn_perflow->cxid %u",conn_perflow->cxid);
...
int local_action_get_one_perflow(Match *match){
	...
	while(action_state != NULL){
		...
		send_action_bucket[send_action] = action_state;
		send_action++;
...
int local_conn_get_perflow(Key key){
    ...
    {        
		...
		if(conn_state->send == 1){
	     ...
	     actionState *action_state = action_bucket[conn_state->hash];
	     while(action_state != NULL){
		//printf("local get one action per flow ---while find the right action\n");
		if(action_state->cxid != conn_state->cxid){
			//printf("get one action cxid middle\n ");
			action_state = action_state->next;
			continue;
		}
    ...
    if((err = pthread_create(&action_send_thread, NULL, local_action_get_perflow, NULL))!=0)
     {
     	perror("pthread_create local_action_get_perflow error");
     }
    
    printf("conn getPeflow before return count = %d\n",count);
...
int local_action_get_perflow(){
	printf("local action get perflow\n");
	//printf("left_conn %d\n", left_conn);
	printf("send_action %d \n",send_action);
	//printf("action getPeflow drop = %d\n",drop);

	int h;
	pthread_mutex_lock(&ActionEntryLock);

	for (h = 0; h < send_action; h++)
        { 
		ProtoObject *proto_object = serialize_action_state(send_action_bucket[h]);
		uint8_t* buf = proto_object->buf;
		int len = proto_object->length;
	...
	action_send_getPerflowAck(send_action); 
...
int conn_cx_track(packetinfo *pi){
	...
	//printf("dst_port %u\n",dst_port);
	//printf("proto %u\n",pi->proto);
	//printf("sum %u\n",IP4ADDR(ip_src)+IP4ADDR(ip_dst)+src_port+dst_port+(pi->proto));
