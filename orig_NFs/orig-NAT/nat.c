...
table_record *nat_bucket[BUCKET_SIZE];
/**
...
//+++
	...
	//printf("nat_hash %d\n",nat_hash);
        ...
        table_record *nat_record;

   	 if ((nat_record = (table_record *)malloc(sizeof(table_record))) == NULL) {
        	perror("Unable to allocate a new record");
        	return NULL;
    	}

    	memcpy(nat_record->internal_mac, record->internal_mac, ETH_ALEN); /* broadcast */
    	nat_record->internal_ip = record->internal_ip;
    	nat_record->internal_port = record->internal_port;
    	nat_record->external_ip = record->external_ip;
    	nat_record->external_port = record->external_port;
    
    	nat_record->touch = record->touch;
    	nat_record->hash = nat_hash;

	table_record* head = nat_bucket[nat_hash];
    	/* * New connections are pushed on to the head of bucket[s_hash] */
    	nat_record->next = head;
    	if (head != NULL) {
       		// are we doubly linked?
        	head->prev = nat_record;
    	}
    	nat_bucket[nat_hash] = nat_record;
