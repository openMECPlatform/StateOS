...
int cxt_update_client(connection *cxt, packetinfo *pi, uint32_t hash)
{
    cxt->last_pkt_time = pi->pheader->ts.tv_sec;
...
int cxt_update_server(connection *cxt, packetinfo *pi, uint32_t hash)
{
    cxt->last_pkt_time = pi->pheader->ts.tv_sec;
...
int cx_track(packetinfo *pi) {
	...
	struct _connection *tmpcxt = (struct _connection *)malloc(sizeof(struct _connection));
    ...
    while (cxt != NULL) {
        ...
        if (af == AF_INET) {
            if (CMP_CXT4(cxt,IP4ADDR(ip_src),src_port,IP4ADDR(ip_dst),dst_port)){
                ...
                return cxt_update_client(cxt, pi, hash);
                ...
                return cxt_update_server(cxt, pi, hash);
            }
        } else if (af == AF_INET6) {
