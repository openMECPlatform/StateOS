...
//+++
extern STLESSStatistics stless_stats;
//+++
...
//+++
	    if (80 == src_port || 80 == dst_port)
     		{ stless_notify_packet_received("HTTP", &recv_time); }
     	    else if (443 == src_port || 443 == dst_port)
     		{ stless_notify_packet_received("HTTPS", &recv_time); }
     	    else
     		{ stless_notify_packet_received("UNKNOWN", &recv_time); }
//+++
            break;
	 ...
	 printf("src_ip %s\n", src_str);
        ...
        if ((record = table_inbound(pi))) {
            ...
            *dst_port = ntohs(record->internal_port);
            ...
            //printf("%-4s src=%u dst=%u\n", ip_protocol_ntoa(ip->protocol),htons(tcp->source), htons(*dst_port));
        }
...
int main(int argc, char **argv)
{
    ...
    //TODO test for stateless controller
    connac_init();
...
//+++
   	...
   	if(statisticGet()!=1){
		printf("statistic get not nothing\n");
    	}
