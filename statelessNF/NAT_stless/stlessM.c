...
int bucketSet(uint32_t hash,struct _table_record *bucketValue){
	const char *v[3];
    size_t vlen[3];
    ...
    vlen[2] = sizeof(struct _table_record);
...
int bucketGet(uint32_t hash, struct _table_record *bucketValue)
{
    redisReply *reply;
    ...
    if (r->len != sizeof(struct _table_record)) {
        printf("get %s\n", tmp);
		freeReplyObject(r);
        return -1;
    }
